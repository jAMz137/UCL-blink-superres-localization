...
from super_resoToolBoxV1 import imloc_max, sci_opt_fit,consec_T, consec_T3,\
                                fit_plot, generate_circle
root        =   'G:\\data2024'
data        =   '\\20240220_2'
targ        =   '53d0'
...
for j in range(0, mark_num):    
    ...
    for i in range(0, np.shape(imlabel)[0]//drift_stp): 
        ...
        label   = np.mean(imlabel[i*drift_stp:
                              (i+1)*drift_stp],axis=0)
        label  -= np.min(label)
        label3  = label/np.max(label)        
        label   = signal.convolve(label3,
                        label30[::-1,::-1],mode='same')                                                        
